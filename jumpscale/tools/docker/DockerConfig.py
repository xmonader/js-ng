...
class DockerConfig:
    def __init__(self, myenv, name, image=None, startupcmd=None, delete=False, ports=None):
        ...
        self._path_vardir = self._tools.text_replace(f"{dirs.BASEDIR}/var/containers/{name}")
        self._tools.dir_ensure(f"{dirs.VARDIR}")
        self._tools.dir_ensure(f"{dirs.BASEDIR}/myhost")
        self._tools.dir_ensure(f"{dirs.BASEDIR}/code")
        self._path_config = f"{self._path_vardir}/docker_config.toml"
        # self.wireguard_pubkey

        if delete:
            self._tools.delete(self._path_vardir)
            self._tools.delete(self._path_config)

        if not self._tools.exists(self._path_config):
    ...
    def load(self):
        if not self._tools.exists(self._path_config):
            raise self._tools.exceptions.JSBUG("could not find config path for container:%s" % self._path_config)

        r = self._tools.config_load(self._path_config, keys_lower=True)
    ...
    def save(self):
        ...
        self._tools.config_save(self._path_config, data)
        assert isinstance(self.portrange, int)
        self.load()
